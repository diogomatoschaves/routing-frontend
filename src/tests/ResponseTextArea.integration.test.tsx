...
describe('Response Textarea works as expected on blur', () => {
  ...
  describe('When an invalid json format / schema is inserted', () => {
    ...
    const SideBarComponent = root.findByProps({ className: "response-textarea" })

    SideBarComponent.props.onClick('', { id: 'responseEdit' })
            
    const TextAreaInstance = root.findByProps({ id: 'responseValue', className: 'textarea' })

    TextAreaInstance.props.onChange('', { id: 'responseValue', value: JSON.stringify({ code: 'OK' }) })
  ...
  describe('When a valid json format / schema is inserted', () => {
    ...
    TextAreaInstance.props.onChange('', { id: 'responseValue', value: JSON.stringify(mockCarResponse) })
    TextAreaInstance.props.onBlur('')
    ...
    it('doesn\'t call the routing api', (done) => {
      expect(routingApi).toBeCalledTimes(0)
      done()
    })
