...
export const Match = {
  Body: {
    ...
    properties: {
      ...
      timestamps: {
        description:
          'Timestamps corresponding to waypoints in seconds since UNIX epoch. Must be monotonically increasing.',
        type: 'array',
        items: {
          type: 'number',
          format: 'int64',
  ...
  Response: {
    ...
    type: 'object',
    required: ['code', 'matchings', 'tracepoints'],
    properties: {
      code: {
        $ref: '/ReturnCode'
      },
  ...
  Matching: {
    id: '/Matching',
    description:
      'A matching object holds the tracepoints and legs of a map-matching response',
    type: 'object',
    required: ['confidence', 'legs'],
    properties: {
      confidence: {
